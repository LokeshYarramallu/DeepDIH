...
class RECLoss(nn.Module):
    def __init__(self):
        ...
        self.deltaX = 1.67
        self.deltaY = 1.67
        self.prop = self.propagator(
            self.Nx, self.Ny, self.z, self.wavelength, self.deltaX, self.deltaY
        )
    ...
    def propagator(self, Nx, Ny, z, wavelength, deltaX, deltaY):
        k = 1 / wavelength
        x = np.expand_dims(
            np.arange(np.ceil(-Nx / 2), np.ceil(Nx / 2), 1) * (1 / (Nx * deltaX)),
            axis=0,
        )
        y = np.expand_dims(
            np.arange(np.ceil(-Ny / 2), np.ceil(Ny / 2), 1) * (1 / (Ny * deltaY)),
            axis=1,
        )
        ...
        return torch.from_numpy(
            np.concatenate(
                [
                    np.real(phase)[np.newaxis, :, :, np.newaxis],
                    np.imag(phase)[np.newaxis, :, :, np.newaxis],
                ],
                axis=3,
            )
        )
    ...
    def forward(self, x, y):
        ...
        cEs = self.batch_fftshift2d(torch.fft.fft2(x, norm="ortho"))
        cEsp = self.complex_mult(cEs, self.prop)

        S = torch.fft.ifft2(self.batch_ifftshift2d(cEsp), norm="ortho")
        Se = S[:, :, :, 0]
